...
/*data (table & content) collected by: https://www.khanacademy.org/profile/patrick809/programs
*/
...
/*Return properties of only object that are ice giants*/
SELECT * FROM solar_system_objects WHERE type_of_object LIKE "%ice giant%";

/*Return count of planets e.a.*/
SELECT COUNT(*), body FROM solar_system_objects WHERE type_of_object LIKE "%planet%" GROUP BY body;

SELECT COUNT(*), body FROM solar_system_objects WHERE type_of_object LIKE "%satellite%" GROUP BY body;
...
/*Return the avg and sum of all objects mass*/
SELECT AVG(mass) FROM solar_system_objects;
SELECT SUM(mass) FROM solar_system_objects WHERE type_of_object LIKE "%planet%";
SELECT SUM(mass) AS total_mass_of_moons FROM solar_system_objects WHERE type_of_object LIKE "%satellite%";


/*Return surface gravity of earth*/
SELECT surface_gravity FROM solar_system_objects WHERE body = "Earth";

/*Return all objects where surface gravity is smaller than the surface gravity of the earth*/
...
SELECT COUNT(*),
    CASE
        ...
        WHEN mean_radius_rel = 1 THEN "Equal to Earth"
...
/*Return densitry & message*/
SELECT density, body,
    CASE
        WHEN density > (SELECT density FROM solar_system_objects WHERE body = "Earth") THEN "More Dense Than Earth"
        WHEN density < 0.50*(SELECT density FROM solar_system_objects WHERE body = "Earth") THEN "Much Less Dense Than Earth"
        WHEN density < (SELECT density FROM solar_system_objects WHERE body = "Earth") THEN "Less Dense Than Earth"
        WHEN density = (SELECT density FROM solar_system_objects WHERE body = "Earth") THEN "Less Dense Than Earth"
    END AS "relative_density_to_earth"
FROM solar_system_objects
GROUP BY body;

SELECT body FROM solar_system_objects WHERE type_of_object LIKE "%planet%" OR type_of_object  LIKE "%ring%" OR type_of_object LIKE "%satellite%";
